...
void UART_Control()
{
  ...
  switch (Uart_Date)
  {
    ...
    case 'B':  RIGHT_1();  M_LOG("우회전 상!\r\n");     break;
    case 'C':  RIGHT_2();  M_LOG("우회전!\r\n");        break;
    case 'D':  RIGHT_3();  M_LOG("우회전 하!\r\n");   break;
    case 'E':  BACK();     M_LOG("후진!\r\n");        break;
    case 'F':  LEFT_3();   M_LOG("좌회전 하!\r\n");    break;
    case 'G':  LEFT_2();   M_LOG("좌회전!\r\n");       break;
    case 'H':  LEFT_1();   M_LOG("좌회전 상!\r\n");  break;
...
이 코드는 네 개의 DC 모터를 제어하여 로봇 자동차를 제어하는 코드입니다. 각 모터에 대한 핀 번호와 방향, 속도를 설정하고, 주어진 입력에 따라 모터를 제어합니다. 적외선 센서를 사용하여 주행 동작을 제어하며, 시리얼 통신을 통해 입력을 받아 동작을 수행합니다.

코드를 실행하기 위해서는 해당하는 핀에 DC 모터 및 센서를 연결하고, Arduino IDE에서 코드를 컴파일하고 업로드해야 합니다. 코드가정상적으로 업로드되면 로봇 자동차가 주어진 동작을 수행하게 됩니다.// 모터 핀 설정
#define PWMA 12    // A 모터 속도
#define DIRA1 34 
#define DIRA2 35  // A 모터 방향
#define PWMB 8    // B 모터 속도
...
// 코드를 실행하기 위해서는 해당하는 핀에 DC 모터 및 센서를 연결하고, Arduino IDE에서 코드를 컴파일하고 업로드해야 합니다. 코드가정상적으로 업로드되면 로봇 자동차가 주어진 동작을 수행하게 됩니다.
