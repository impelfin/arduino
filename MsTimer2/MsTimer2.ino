...
void loop() {     
   ...
   if(digitalRead(SW1) == LOW) {  
     if(sw1state1 == true) { 
       if(Count01 >= 1) { sw1state1 = false;
          if(sw1state) { sw1state = false; digitalWrite(LED1, LOW); } 
           else  { sw1state = true; Count1 = 0; }}
     }  else  { sw1state1 = true; Count01 = 0; }
   }
   if(digitalRead(SW2) == LOW){  
     if(sw2state1 == true) { 
        if(Count02 >= 1) { sw2state1 = false;
          if(sw2state) { sw2state = false;  digitalWrite(LED2, LOW); } 
          else { sw2state = true; Count2 =0; }}
     }  else  { sw2state1 = true; Count02 = 0; }
   }
